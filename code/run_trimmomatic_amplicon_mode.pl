...
foreach my $name (sort keys %paired_files){
	...
	my $cmd="java -jar  /c1/apps/trimmomatic/Trimmomatic-0.39/trimmomatic-0.39.jar PE -trimlog out.trimlog -phred33 -threads 2 $paired_files{$name}[0] $paired_files{$name}[1] $path$name"."_R1_paired_trimmed.fastq $path$name"."_R1_single_trimmed.fastq $path$name"."_R2_paired_trimmed.fastq $path$name"."_R2_single_trimmed.fastq LEADING:3 TRAILING:3 SLIDINGWINDOW:4:15 MINLEN:225";
	print $cmd,"\n";
	die if system($cmd);
}
